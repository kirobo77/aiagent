...
MCP는 JSON-RPC 2.0 표준을 기반으로 하며, 다음과 같은 확장을 제공한다.
...
# 2. LTS 버전 다운로드 (안정성이 좋음)
# 3. .msi 파일 실행하여 설치
# 4. 설치 완료 후 확인
### Linux
# sudo apt update
# sudo apt install nodejs npms
...
Resource Templates에서 get_greeting을 Tools에서 add를 확인 가능하다.
...
1. API 키 생성 버튼 클릭
   - "API 키 받기" 섹션으로 이동하여 "API 키 생성" 버튼을 클릭 [Az
2. 프로젝트 선택
   - 새 프로젝트에서 API 키 생성 옵션을 선택
   - 기존 Google Cloud 프로젝트가 있으면 해당 프로젝트 선택 가능
3. 샘플소스 내 llm 설정에 아래 내용으로 변경

```
config_list_gemini = [
    {
        "model": "gemini-1.5-pro",
        "api_key": "AIzaSyD7w3YrqCJtLnCI6XLeVlEqDOky0k-Tims",
        "api_type": "google",
